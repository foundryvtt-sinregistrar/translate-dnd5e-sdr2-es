(async () => {
    ...
    const extractEffects = (doc) => {
        ...
        if (!arr.length) return {}; // siempre objeto
    ...
    const extractAdvancement = (doc) => {
        ...
        // Siempre devolvemos objeto (aunque vacío)
    ...
    for (const id of ids) {
        ...
        const key = doc.id; // clave phb...
        if (!key) continue;

        // SIEMPRE existen estas 3 keys (aunque vacías)
        const entry = {
            name: doc.name ?? "",
            description: "",
            activities: {},
