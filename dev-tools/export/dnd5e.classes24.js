(async () => {
    ...
    const extractAdvancement = (doc) => {
        ...
        if (!Array.isArray(adv) || !adv.length) return undefined;
    ...
    for (const id of ids) {
        ...
        const acts = extractActivities(doc);
        if (acts) entry.activities = acts;

        const effs = extractEffects(doc);
        if (effs) entry.effects = effs;

        const adv = extractAdvancement(doc);
        if (adv) entry.advancement = adv;

        entriesTmp[key] = entry;

        i++;
        if (i % 100 === 0) console.log(`[${PACK_ID}] ${i}/${ids.length}`);
    }
